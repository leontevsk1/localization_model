...
// RecomputeDistances пересчитывает матрицу измеренных расстояний по текущим RealCoord
// с добавлением шума. Вызывается на каждом тике при временной динамике.
func RecomputeDistances(nodes []*types.Node, distErr float64, rng *rand.Rand) [][]float64 {
	...
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			dx := nodes[i].RealCoord.X - nodes[j].RealCoord.X
			dy := nodes[i].RealCoord.Y - nodes[j].RealCoord.Y
			dz := nodes[i].RealCoord.Z - nodes[j].RealCoord.Z
			trueDist := math.Sqrt(dx*dx + dy*dy + dz*dz)
